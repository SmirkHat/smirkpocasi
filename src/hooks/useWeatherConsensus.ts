...
import { extractForecastSeries } from '../utils/forecastSeries';
import {
...
const CACHE_KEY = 'smirkpocasi:last-consensus:v16';
const FIELDS = [
  'temperature',
  'apparentTemperature',
  'dewPoint',
...
const OBSERVATION_SOURCE_IDS = new Set([
  'opensensemap',
  'chmi',
  'aviation',
  'brightsky',
...
function withDerivedMetrics(value) {
  const next = { ...value };
...
function publicSource(provider, value, status) {
...
function locationInCoverage(provider, location) {
...
function fetchWithTimeout(provider, requestLocation) {
  if (!locationInCoverage(provider, requestLocation)) {
    return Promise.resolve({
      providerStatus: 'not-applicable',
      error: 'Zdroj nemá pro tuto polohu pokrytí.'
    });
  }

  const key = `${provider.id}:${requestLocation.lat}:${requestLocation.lon}`;
  const inFlight = inFlightProviderRequests.get(key);
  if (inFlight) return inFlight;

  let timeoutId;
  const timeout = new Promise((_, reject) => {
    timeoutId = window.setTimeout(() => reject(new Error(`${provider.name} timeout.`)), PROVIDER_TIMEOUT_MS);
  });

  const request = Promise.race([provider.fetchFn(requestLocation, provider), timeout]).finally(() => {
    window.clearTimeout(timeoutId);
    inFlightProviderRequests.delete(key);
  });
  inFlightProviderRequests.set(key, request);
...
function consensusCacheMatchesLocation(cached, location) {
...
export function useWeatherConsensus(location) {
  ...
  const initialRelevant = consensusCacheMatchesLocation(initial, location);
  const empty = { consensus: null, sources: pendingSources(), divergence: 0, confidence: 'low', aladin: null, yr: null, forecastSeries: [] };
  const [state, setState] = useState(() => (initialRelevant ? initial : empty));
  const [loading, setLoading] = useState(() => !(initialRelevant && initial?.consensus));
  const [error, setError] = useState(null);

  useEffect(() => {
    if (!location?.lat || !location?.lon) return;

    const requestLocation = { lat: location.lat, lon: location.lon };
    let ignore = false;
    const cached = readCache();
    if (!consensusCacheMatchesLocation(cached, location)) {
      setState(empty);
      setLoading(true);
    }
    setError(null);
    ...
    const enabledProviders = PROVIDERS.filter((provider) => provider.enabled);
    Promise.allSettled(enabledProviders.map((provider) => fetchWithTimeout(provider, requestLocation)))
      .then((settled) => {
        if (ignore) return;
        const next = {
          ...buildResult(settled),
          requestLat: requestLocation.lat,
          requestLon: requestLocation.lon
        };
        try {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- stejná logika jako useWeather (jen lat/lon).
