...
| **Backend / API** | **26 Vercel-style serverless handlers** in repo-root `/api/*.js` (Node `(req, res)` shape with `req.query` / `res.json`). Locally wired by a custom Vite middleware plugin in `vite.config.js` (`localApiPlugin`). Production: Vercel Serverless Functions + `vercel.json` SPA rewrite to `index.html`. Client calls relative `/api/...`. Some client fetches hit public third-party APIs directly (e.g. OpenSenseMap, Bright Sky, Open-Meteo AQI). |
